...
if skipTo != 2:
	cmd = 'cd ..; python '+os.environ['DPSAMPLER']+'scripts/runDMC.py '+fpath+' s '+str(numSamples)+' -1 '+DPSampleFpath
...
if skipTo!=None:
	print 'Skip to argument is not None. So assuming at least one of the sample files are already present and computing distributions..'
	DPSampleFpath = os.environ['TMP']+'/'+fname+'.dpsamples'
	idealSampleFpath = os.environ['TMP']+'/'+fname+'.idealsamples'
...
pl.legend((p1,p2),('DPSampler', 'Ideal Sampler'),loc='best',fontsize=12)
...
pl.savefig('distributions.eps',bbox_inches='tight')
pl.show()
