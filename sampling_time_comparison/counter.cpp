...
Float Counter::writeSamples(const Cnf &cnf, string sampleFile, Int numSamples){
  ...
  if (indEmpty != DUMMY_MIN_INT) { // empty clause found
    ...
    Int nTSteps = 10, nT = 1;
    TimePoint samplerStart = util::getTimePoint();
    ...
    TimePoint endSamplerTime = util::getTimePoint();
    Float timeTaken = std::chrono::duration_cast<std::chrono::milliseconds>(endSamplerTime - samplerStart).count() / 1000.0; 
    printComment("Sample Generation Time: "+to_string(timeTaken));
    printComment("Time per sample: "+to_string(timeTaken/numSamples));
